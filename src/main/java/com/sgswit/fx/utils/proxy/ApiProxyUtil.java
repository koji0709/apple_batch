    ...
    private static Entity getRandomOne(){
        ...
        try{
            Db.use(DataSourceFactory.getDataSource()).tx(db->{
                ...
                Entity queryOne= Db.use(DataSourceFactory.getDataSource()).queryOne("SELECT * FROM proxy_ip_info where expiration_time>? and (last_update_time+3*1000<?) ORDER BY expiration_time LIMIT 1",currentTimeMillis,currentTimeMillis);
                if(null!=queryOne){
                    one.set(queryOne);
